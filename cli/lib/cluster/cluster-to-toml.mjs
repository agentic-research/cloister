...
export async function main(argv = process.argv.slice(2), deps = {}) {
  ...
  if (writeIdx >= 0 && !writePath) {
    (deps.errLog ?? console.error)("cluster-to-toml: --write requires a path argument");
    return 1;
  }

  const sourcePath = (deps.env ?? process.env).CLUSTER_TS ?? DEFAULT_CLUSTER_TS;
  const sourceModule = await tsImport(pathToFileURL(sourcePath).href, { parentURL: import.meta.url });
  const cluster = sourceModule.cluster;
  if (!cluster) {
    (deps.errLog ?? console.error)(`cluster-to-toml: ${sourcePath} does not export 'cluster'`);
    return 1;
  }
  ...
  if (writePath) {
    ...
    (deps.errLog ?? console.error)(`cluster-to-toml: wrote ${rel}`);
  } else {
    (deps.stdout ?? process.stdout).write(toml);
  }
  return 0;
}

...
import { resolveBundleImage } from "../oci-artifact.mjs";
...
export function emitCompose(cluster, ociByInput = new Map(), opts = {}) {
  ...
  const lines = [];
  lines.push(`# AUTO-GENERATED by cloister cluster generate. Do NOT edit by hand.`);
  lines.push(`# Regenerate via \`cloister cluster generate\` after editing cluster.toml.`);
  lines.push(`#`);
  ...
  for (const b of cluster.bundles) {
    ...
    if (tenants.length > 0) {
      for (const tenant of tenants) {
        emitBundleContainer(lines, b, cluster, colocation, tenant, perTenantDoVolumes, ociByInput, doBindPath, ociByBundle, warn);
      }
    } else {
      emitBundleContainer(lines, b, cluster, colocation, null, perTenantDoVolumes, ociByInput, doBindPath, ociByBundle, warn);
    }
  }
 ...
 */
function resolveBundleImageOrWarn(ext, colocatedInputs, ociByInput, bundleName, ociByBundle = new Map(), warn = console.warn) {
...
function emitBundleContainer(lines, b, cluster, colocation, tenant, perTenantDoVolumes, ociByInput = new Map(), doBindPath = "", ociByBundle = new Map(), warn = console.warn) {
  ...
  const image = resolveBundleImageOrWarn(ext, colocatedInputs, ociByInput, b.name, ociByBundle, warn);
...
export function parseOciPins(lockText) {
  const lock = parseToml(lockText);
  ...
  for (const [name, row] of Object.entries(lock.inputs ?? {})) {
    ...
    for (const bundle of row?.ociBundles ?? []) {
      if (bundle && typeof bundle === "object" && bundle.bundle && bundle.identifier) {
        ociByBundle.set(bundle.bundle, bundle);
      }
    }
  }
  return { ociByInput, ociByBundle };
}
...
export async function main(_argv = process.argv.slice(2), deps = {}) {
  ...
  const INPUT_PATH = env.CLUSTER_TS ?? resolve(REPO, "src/generated/cluster.ts");
  const OUTPUT = env.COMPOSE_OUTPUT ?? resolve(REPO, "cluster.compose.yaml");

  if (!existsSync(INPUT_PATH)) {
    errLog("emit-compose: failed to load cluster manifest");
    errLog(`  tried: ${INPUT_PATH}`);
    errLog("  run `cloister cluster generate` first");
    return 1;
  }

  let mod;
  try {
    mod = await tsImport(pathToFileURL(INPUT_PATH).href, { parentURL: import.meta.url });
  } catch (error) {
    errLog("emit-compose: failed to import cluster manifest");
    errLog(error?.message ?? error);
    return 1;
  }
  ...
  const { validateCluster } = await tsImport(
  ...
  const LOCKFILE = env.CLOISTER_LOCKFILE ?? resolve(REPO, "cluster.lock.toml");
  if (existsSync(LOCKFILE)) {
    try {
      ({ ociByInput, ociByBundle } = parseOciPins(readFileSync(LOCKFILE, "utf8")));
    } catch (e) {
      errLog(`emit-compose: could not read oci packages from ${LOCKFILE}: ${e.message}`);
    }
  }

  let body;
  try {
    body = emitCompose(cluster, ociByInput, { doBindPath: env.CLOISTER_DO_BIND || "", ociByBundle });
  } catch (e) {
    errLog(e.message);
    return 1;
  }

...
function renderCapnp(g) {
  const lines = [];
  lines.push("# cloister.capnp — AUTO-GENERATED by cloister cluster generate");
  ...
  lines.push("# To regenerate: `cloister cluster generate`.");
  lines.push("");
...
export async function main(argv = process.argv.slice(2), deps = {}) {
  ...
  const writePath = writeIdx >= 0 ? args[writeIdx + 1] : (env.CLOISTER_OUTPUT || DEFAULT_OUTPUT);

  const tomlPath = env.CLUSTER_TOML ?? DEFAULT_TOML;

  let cluster;
  try {
    cluster = await parseTomlToCluster(readFileSync(tomlPath, "utf8"));
  } catch (e) {
    errLog(`emit-cloister-capnp: ${e.message}`);
    return 1;
  }
  ...
  if (writePath) {
    ...
    errLog(`emit-cloister-capnp: wrote ${rel}`);
  } else {
    (deps.stdout ?? process.stdout).write(out);
  }
  return 0;
}

#!/usr/bin/env node
// `cloister init` scaffold subcommand (cloister-ca1dab).
//
// Usage:
//   cloister init --recipe <name> --out <dir> [--port N] [--force]
//   cloister init --recipe <name> --out <dir>
//
...
// Lives in `cli/` rather than the Worker bundle under `src/` because it uses
...
// Recipes live at <repo-root>/recipes/. findRecipesRoot deliberately walks up
// from this product module so source and installed layouts share one path.

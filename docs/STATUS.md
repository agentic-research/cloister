...
|---|---|---|---|
| **`cloister/credential-isolation/v1`** capability | **ADR-0024**, `cloister-spec/credential-isolation/v1/`, `docs/plans/credential-isolation-capability.md` | **`cloister-8f57f0`** | First concrete capability under the substrate-as-kernel framing. **ALL 8 PHASES + credential-store seam + ROUTE MOUNT + MANIFEST EXTENSION shipped 2026-05-18** (PRs #29-#34, #36, #37). Operator now declares services in `cloister.capnp` via `vaultProxyServices`; route mounts via `vaultProxy` Route.kind variant. 1068 tests green (69 in the cred-iso suite: 34 handler unit + 9 route integration + 26 manifest conversion + 5 e2e fixture). 16 Copilot findings across two review passes — all addressed + resolved. Pending: production credential store (vault-DO-backed impl, last load-bearing piece). |

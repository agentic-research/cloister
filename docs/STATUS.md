...
| **ADR-0033** — bd as cloister-mediated bead substrate | **ADR-0033** (Proposed 2026-06-23) | **`cloister-9d19e3`** (design), **`cloister-c2bd47`** (impl) | Status: Proposed. Cloister adopts `bd` (the dolt-sql-server-backed bead tracker with MCP server + socket mode) as an external `mcpProxy` substrate — structurally identical to mache + llo. No new backend kind, no MySQL client in workerd. D1: `mcpProxy` backend with `handlesPrefix=bd_` + `claims=[bd_create, bd_search, bd_update, bd_ready, bd_dep, bd_prime, bd_remember]`. D2: bd MCP server runs as a cluster bundle (sidecar) with `--listen-uds /run/cloister-uds/bd.sock`. D3: Wire is HTTP MCP over UDS, not MySQL. D4: Auth via shared-secret bearer token mediated by vault per ADR-0024 cred-iso/v1 (LLO ADR-0022 precedent). D5: `.beads/dolt/cloister/` migration deferable — rsry + bd coexist. D6: explicit multi-substrate framing — cloister is substrate-of-substrates; bd slots into the existing pattern (mache, llo, notme). **No dependency on rosary**; cloister's adoption is orthogonal to rosary's substrate decisions. Implementation deferred to `cloister-c2bd47`; sibling `notme-9da488` parked (bearer-token choice eliminates LocalCA extension; kept for future TLS-client-cert Option B). |

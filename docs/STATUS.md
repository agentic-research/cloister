...
|---|---|---|---|
| **`cloister/credential-isolation/v1`** capability | **ADR-0024**, `cloister-spec/credential-isolation/v1/`, `docs/plans/credential-isolation-capability.md` | **`cloister-8f57f0`** | First concrete capability under the substrate-as-kernel framing. **ALL 8 PHASES shipped 2026-05-18** (PRs #29-#32). **Credential-store seam shipped** (PR #33) — `CredentialStore` interface + `InMemoryCredentialStore` dev impl. Pending: route mount (compose handler ← credential-store ← service-config in `src/index.ts` + declare in `cloister.capnp`), production credential store (vault-DO-backed impl), manifest extension (`VaultProxyService` from `cluster.toml`). |

...
| `cloister-204ac9` | Bundle type drift gate (hand-maintained ↔ generated) | partial-shipped 2026-06-24 (commit `66484e5`) — `src/manifest/bundle-drift-guard.ts` provides compile-time `Normalize<T>`-equality gate via `task lint`'s tsc pass. Full consolidation (delete `cluster-types.ts`) deferred until `cloister-818f2b` adds schema-bridge readonly + JSDoc emission. Bead stays open as P3 tracker. |
| `cloister-818f2b` | schema-bridge: emit `readonly` for array fields + carry capnp `# comments` as JSDoc | P3 — filed 2026-06-24 as `cloister-204ac9` followup. Two emitter deltas (readonly modifier + comment carry-through) needed before the hand-maintained `cluster-types.ts` can be deleted in favor of `cluster.zod.ts` as single source of truth. |

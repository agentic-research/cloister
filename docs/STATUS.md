...
| **ADR-0038** — Derive bundle image from `server.json` `packages[].oci` | **ADR-0038** (Proposed 2026-07-03) | **`cloister-3c4b0c`** (design + STATUS row); producer side via **`cloister-31a988`** (mache constellation) | Status: Proposed. Extends ADR-0026's input→manifest derivation from *backends* (`_meta.art.cloister/v1`) to the *runtime image*: a bundle's `image` may be derived from the linked input's resolved `server.json` `packages[]` entry where `registryType == "oci"`. Precedence, loud: operator `ext.image` wins → else derive `<identifier>:<version>` (or `@<digest>`) from `packages[].oci` → else stderr warning naming bundle+input (no silent empty image). Producer side (mache `server.json` gains an `oci` `packages[]` entry via `tools/server-json-gen`) lands independently and is standard-MCP-valid on its own; consumer side (resolve-inputs records the oci ref into `cluster.lock.toml`; emit-compose + emit-workerd-config derive image; new fail-loud bundle-isolation invariant) is the impl bead to file. Open: publish-pipeline verification (declared ref must be pullable) + digest-pin recommendation. Note: ADR-0037 reserved for secure MCP ingress transports (`cloister-22a5ca`, per `docs/deployment/secure-art-tools.md`). |

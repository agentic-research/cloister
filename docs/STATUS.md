...
| **ADR-0031** — `cloister.capnp` as build artifact | **ADR-0031** | **`cloister-345ad1`** | Phase 2 of the "cloister.capnp as build artifact" arc — `cluster.toml [[routes]]` + emitter (`scripts/emit-cloister-capnp.mjs`) make `cloister.capnp` a derived artifact, joining `cluster.ts` + `manifest.ts` + `cluster.zod.ts` in the generated column. One operator surface (`cluster.toml`) at the repo root. Phase 3 (per-recipe migration) + Phase 4 (per-upstream backend retirement to lockfile pattern) deferred to separate beads. Pairs with ADR-0025 (bidi pipeline) + ADR-0026 (lockfile `[[generated_backends]]` overlay layer). |

...
| **ADR-0026** — Tool composition model (Nix-flakes-shaped, MCP-registry-resolved, content-addressed, Interlace-signed) | **ADR-0026**, `cluster.toml` `[inputs.*]` block (Phase 1a + 1b shipped) | **`cloister-cf7a3b`** | Operator declares tools by `ref = "io.github.org/tool"` + `version = "^0.1"`; cloister resolves via registry, fetches signed `server.json`, composes wiring. **Phase 1a (PR #62)** + **Phase 1b (PR #63)** shipped — `[inputs.*]` schema seam + file/https resolver + `cluster.lock.toml`. Phases 2 (registry), 3 (signature verify), 4 (matchmaker — per ADR-0027) remaining. |

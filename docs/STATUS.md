...
| **ADR-0039** — Securing local Durable Object SQLite at rest | **ADR-0039** (Accepted 2026-07-05) | **`cloister-ffd17b`** (research + ADR + impl) | Status: Accepted — macOS Phase 0/1 shipped + proven on real hardware (`task do:harden`, `KEK_HELPER` config.capnp binding, `task dev:securevol`, `task do:migrate-securevol`, `CLOISTER_DO_BIND` host bind-mount); Linux twin + `serve:local` preflight remain. Local `workerd serve` / miniflare DO SQLite sits plaintext (mode 0644) — bead/trust/blob/vault state — and workerd's `durableObjectStorage` union (none/inMemory/localDisk) has no encryption hook. Refines the §13.7.4 operator-tier disk boundary for the local case: on an AI-agent machine any same-UID process is de-facto operator-tier (the ADR-0024 adversary). Three composable layers, Mac-first: (1) at-rest encryption via OS-keystore KEK — the vault schemes (`keychain://`/`apple-password://` Mac, `secret-tool://`/`keyring://` Linux) already provide custody; Phase 1 = encrypted APFS volume mounted at `CLOISTER_DO_PATH` + Keychain passphrase, covers WAL/-shm; (2) BLAKE3 integrity manifest over canonical rows, anchored outside the FS → silent tamper becomes detected-at-boot; (3) selective column sealing (generalize the §18 vault envelope, HKDF sub-keys). Phase 0 (`chmod 700` + Time Machine exclusions) is zero-code today. Closes two gaps: missing `KEK_HELPER` binding in `config.capnp`; plaintext `vault_state` KEK-pin re-pin (`cloister-fbc6eb`). Threat model §13.9 seeds the seam. Linux twin = fscrypt/LUKS + `secret-tool://`. |
| **ADR-0040** — Harness-in-cloister: cloister as the audited control + credential plane | **ADR-0040** (Proposed 2026-07-06) | **`cloister-f3e3ae`** (this ADR frames the L1 build) | Status: Proposed. Formalizes what "the harness runs in cloister" means given workerd has no process-spawn (ADR-0033). Three layers: **L0** orchestration mediation — rosary's MCP face in cloister, `rsry_dispatch` through `/mcp`, auth'd + receipted (SHIPPED via `cloister-cf7a3b`); **L1** credential proxy — the harness's LLM key lives in the vault, `ANTHROPIC_BASE_URL` → cloister, key injected + SSE streamed back + receipt per call (the `cloister-f3e3ae` build); **boundary** — agent processes + the harness CLI run HOST-SIDE (compute sandboxing is ADR-0009 microVM territory, deferred). Custody claim precise: TRUE for API-key/enterprise shapes; Max/Pro OAuth gets audit (receipts), not custody. Main risk: SSE streaming fidelity (no buffering). Threat model gains a section before L1 lands. |

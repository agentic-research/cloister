...
| **§13.4 audit reconstitution query** | Post-migration, the bead row in rsry/bd's Dolt has no `content_hash` column. The audit chain reconstitutes via the `bead_id` link to TrustStore. Two access paths: **(a) RPC** — `env.TRUST_STORE.get(env.TRUST_STORE.idFromName("cluster")).attestationsForBead(beadId)` returns `PeerAttestation[]`. First-class substrate surface, no direct storage access required. **(b) Direct SQL** for operator forensics with disk-level access: <br><br>```sql<br>-- For a bead, recover the (content_hash, scope, peer_fp) audit tuple<br>SELECT a.content_hash, a.scope, a.peer_fingerprint, a.created_at<br>FROM peer_attestations a<br>WHERE a.bead_id = '<bead-id>'<br>ORDER BY a.created_at ASC<br>;<br>```<br><br>Helper: `attestationsForBead(sql, beadId): PeerAttestation[]` (`src/storage/peer-attestations.ts`). RPC method added 2026-06-24 (commit 7d02708). Beads created via direct rsry bypass (no orchestrator) have no attestation row — same posture as direct DO-RPC bypass today. |

...
//
// confinement-digest — compute the `confinementDigest` (BLAKE3-256 of the §6
// CANONICAL ConfinementManifest bytes, per cloister/confinement/v1 §6/§7) using
...
// claim commits to (§7) and cloister's half of the §8.2 cross-impl conformance.
//
...
// varied) reaches the same digest — that's what §6 buys.
//
...
/// §6 canonical serialization: parse → `serde_json::Value` (default Map is a
/// sorted `BTreeMap`, giving §6.2 ASCII-sorted keys at every level) →
/// `to_string_pretty` (2-space indent, §6.4) → strip the trailing newline so
/// the last byte is `}` (§6.3). Null-field omission (§6.4) is inherent: input
